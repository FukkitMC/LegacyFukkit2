...
public class AntiXray
{
     ...
     */
    public void updateNearbyBlocks(World world, BlockPos position)
    {
                        ...
                        for ( int z = 0; z < 16; z++ )
                        {
                            for ( int x = 0; x < 16; x++ )
                            {
                                ...
                                if ( obfuscateBlocks[blockId] )
                                {
                                    ...
                                    if ( !isLoaded( world, new BlockPos( startX + x, ( i << 4 ) + y, startZ + z ), initialRadius ) )
                                    {
                                        index++;
                                        continue;
                                    }
                                    // On the otherhand, if radius is 0, or the nearby blocks are all non air, we can obfuscate
                                    if ( !hasTransparentBlockAdjacent( world, new BlockPos( startX + x, ( i << 4 ) + y, startZ + z ), initialRadius ) )
                                    {
                                        int newId = blockId;
    ...
    private void updateNearbyBlocks(World world, BlockPos position, int radius, boolean updateSelf)
    {
        ...
        if ( world.blockExists( position ) )
        {
            ...
            if ( updateSelf && obfuscateBlocks[Block.getIdByBlock( block )] )
            {
                // Send the update
                world.method_411( position );
            }
    ...
    private static boolean isLoaded(World world, BlockPos position, int radius)
    {
        return world.blockExists( position )
                && ( radius == 0 ||
    ...
    private static boolean isSolidBlock(Block block) {
        ...
        // blocks around them.
        return block.isFullCube() && block != Blocks.SPAWNER && block != Blocks.BARRIER;
    }
}
